...
//
// Priority order:
//   1. If `GEMINI_API_KEY` is set -> generate anime-style background with Gemini
//      (best quality but slow + costs tokens).
//   2. If a pre-baked PixelLab PNG is present in /public/bg/ -> use it.
//   3. Otherwise fall back to a rich CSS gradient data-URL that paints an
//      anime-style biome (instant, offline, AGENTS.md-compliant: no real-life
//      photos, no broken external links).
//
// This replaces the old approach which used real photos from
// play.pokemonshowdown.com/fx/bg/*.jpg -- those violated AGENTS.md's
// "never use real-life photos" rule and are referer-blocked anyway.
// =============================================================================
...
export const generateBattleBackground = async (
    ...
    // Multiplayer / no key -> static (instant, deterministic)
